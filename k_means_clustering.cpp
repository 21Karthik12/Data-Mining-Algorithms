...
private:
	...
	void print_info(int iter_num) {
		cout<<"After iteration - "<<iter_num<<":\n";
		for(int i = 0; i < k; i++) {
			cout<<"Centroid of cluster-"<<(i + 1)<<": (";
			cout<<centroid[i].first<<", "<<centroid[i].second<<")\n";
		}
		for(int i = 0; i < n; i++) {
			cout<<"Cluster number of data point ("<<data[i].first;
			cout<<", "<<data[i].second<<"): "<<(cluster_number[i] + 1)<<"\n";
		}
		cout<<"\n";
	}
...
int main() {
	...
	vector<pair<double, double>> data;
	ifstream inp;
	inp.open("datasets/k_means_clustering_input.txt");
	int size; inp>>size;
